...
HandlerDescriptor::HandlerDescriptor(HandlerDescriptor* nextHandler)
  ...
  if (nextHandler == this) { // initialization 仅仅用于创建一个新的双向链表，在HandlerSet的构造函数中
    fNextHandler = fPrevHandler = this;		//gxq 只有在HandlerSet构造函数中初始化时才会走这里
  } else {
    fNextHandler = nextHandler;			//gxq  链表的头部插入法，即每增加一个节点，都是增加在头部的后面
...
HandlerSet::HandlerSet()
  : fHandlers(&fHandlers) {		//gxq 这里这样初始化是因为HandlerDescriptor的构造函数需要参数
...
::assignHandler(int socketNum, int conditionSet, TaskScheduler::BackgroundHandlerProc* handlerProc, void* clientData) {
  ...
  if (handler == NULL) { // No existing handler, so create a new descr:
    handler = new HandlerDescriptor(fHandlers.fNextHandler);//头节点的下一个节点
